(function(root){
 ...
 function account(s){
  const market=s.pending?.market,p=(market?s.players.find(p=>p.id===market.eligibleTraders?.[market.participantIndex]):null)||s.players[s.turn],d=$('tradeAccount'),expanded=d.querySelector('details')?.open;d.replaceChildren();d.append(el('strong',p.name+' · '+p.occupation+(market?.status==='trading'?' · 目前交易玩家':'')));
