...
const jobs=session();jobs.state.players.forEach((p,i)=>p.employment={industry:i<2?'紡織':'食品',isHighestRank:false});const salaries=jobs.state.players.map(p=>p.salaryIncome);open(jobs,'stock-news-017',2);assert.deepEqual(jobs.state.players.map(p=>p.salaryIncome),[0,0,salaries[2],salaries[3]]);
const highest=session();highest.state.players.forEach((p,i)=>p.employment={industry:'金融',isHighestRank:i===0});open(highest,'stock-news-029',1);assert.ok(highest.state.players[0].salaryIncome>0);assert.ok(highest.state.players.slice(1).every(p=>p.salaryIncome===0));
...
const explicitOccupation=session();explicitOccupation.state.players[0].occupation='紡織公司工程師';open(explicitOccupation,'stock-news-017',2);assert.equal(explicitOccupation.state.players[0].salaryIncome,0);
