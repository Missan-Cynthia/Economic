...
for(let trigger=0;trigger<4;trigger++){
 ...
 unchanged(s,()=>s.end());unchanged(s,()=>s.finish());unchanged(s,()=>s.skip());unchanged(s,()=>s.apply());unchanged(s,()=>s.borrow(100,1));unchanged(s,()=>s.chooseBankLoan(100,1,s.bankPhase.id));
 const id=s.bankPhase.id;s.chooseBankLoan(100000);assert.equal(s.state.players[0].cash,before[0].cash+100000);assert.ok(Math.abs(s.state.players[0].monthlyCashflow-(before[0].monthlyCashflow-166.67))<1e-8);
 unchanged(s,()=>s.chooseBankLoan(100000,0,id));assert.equal(s.bankApplicant.id,1);
 ...
 restored.declineBankLoan();assert.equal(restored.bankApplicant.id,2);restored.chooseBankLoan(200000);assert.equal(restored.state.phase,'resolving');restored.declineBankLoan();assert.equal(restored.state.phase,'done');assert.equal(restored.state.turn,trigger);
 ...
 unchanged(restored,()=>restored.chooseBankLoan(1,3,id));restored.end();assert.equal(restored.state.turn,(trigger+1)%4);
}
...
for(const principal of [300001,0,-1,NaN,undefined])unchanged(limits,()=>limits.chooseBankLoan(principal));
limits.chooseBankLoan(300000);assert.equal(limits.state.players[0].debtTotal,500000);limits.state.players[1].maxLoan=0;assert.equal(limits.bankLoanLimit(),0);unchanged(limits,()=>limits.chooseBankLoan(1,0));limits.declineBankLoan();
